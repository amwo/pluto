...
| 8 → 8-c | live exit > entry priority | check_exits live が in-flight の間 handle_buy live を待たせる Mutex / Semaphore 設計。Jupiter throttle (2s) と check_exits tick (30s) でほぼ自然 serialize されるが、明示的な優先度制御は live-readiness で必要 | 引き取り先: 8 phase 8-c |
