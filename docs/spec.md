...
| 4 → 7 phase 7d | spec 5.3 cold streak filter | `ObservedTrades::target_recent_pnl_lamports(target, 1h)` で target の直近1h SOL delta 集計、`< -300_000_000` で `SkipReason::TargetColdStreak`。`mint_blocklist` は同 mint 抑制、cold streak は wallet 全体の調子悪検知 | ✅ done (milestone 7 phase 7d), proxy 不正発覚 → デフォルト無効化 (phase 7e) |
| 7 phase 7e | filter param 再調整 (実データ反映) | 過去 50 件 target trade 分析: P50 buy = 1 SOL、P90 = 2 SOL、最大 5 SOL。max_target_buy 0.7 → **2.5 SOL** (5 SOL 異常のみ弾く)、copy_ratio 25% → **10% (1/10)**、max_copy 0.15 → **0.2 SOL**、cold_streak threshold = `i64::MIN` で **無効化** (SOL delta proxy が accumulation 中 false cold)。`SkipReason::InsufficientBalance` 追加、`FilterContext.wallet_balance_lamports` を pure decide() で check (size + 0.015 SOL buffer)。dry でも記録され、live 移行時の preflight 兼ねる | ✅ done (milestone 7 phase 7e) |
...
| 8 phase 8-d | Solana+Jito 専門家レビュー全件対応 | batch1: Jupiter `dynamicComputeUnitLimit` + auto priority fee, blockhash gate, balance preflight, sig_count > 1 reject, race endpoints (300ms stagger), commitment=confirmed。batch2: live_send_attempts テーブル + 冪等 retry。batch3: solana-sdk 採用 (ed25519-dalek 退役、zeroize conflict 解消)。batch4: `/api/v1/bundles` 切替、tip ix + jitodontfront marker を別 tx で同 bundle、tip account 8口座 random rotation、`bundles.jito.wtf/api/v1/bundles/tip_floor` p75 動的取得 (fallback 100k lamports)、ATA close (spl-token CloseAccount discriminator=9 + find_program_address で ATA 派生) を sell-all 後に別 bundle 送信 | ✅ done (milestone 8 phase 8-d) |
